...
SELECT Date(main.entryTime) as entryTime, product.pName,product.price, Count(main.barcode) AS Total FROM main INNER JOIN product ON main.barcode=product.barcode WHERE main.userID = "12345"  group by main.barcode;
SELECT main.entryTime, product.pName,product.price, main.barcode FROM main INNER JOIN product ON main.barcode=product.barcode WHERE main.userID = "12345" group by main.barcode;
